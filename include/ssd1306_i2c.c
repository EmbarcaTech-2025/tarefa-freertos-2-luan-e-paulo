...
// Desenha um círculo preenchido com centro (x0, y0) e raio radius
// Desenha um círculo preenchido no display
void ssd1306_draw_circle(uint8_t *ssd, int x_0, int y_0, int radius, bool set) {
    int x = radius;
    int y = 0;
    int err = 0;
    ...
    while (x >= y) {
        ...
        for (int i = x_0 - x; i <= x_0 + x; i++) {
            ssd1306_set_pixel(ssd, i, y_0 + y, set);
            ssd1306_set_pixel(ssd, i, y_0 - y, set);
        }
        for (int i = x_0 - y; i <= x_0 + y; i++) {
            ssd1306_set_pixel(ssd, i, y_0 + x, set);
            ssd1306_set_pixel(ssd, i, y_0 - x, set);
        }
